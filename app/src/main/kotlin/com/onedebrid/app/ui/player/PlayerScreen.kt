 ...
 *
 * request/profileId are accepted as plain parameters rather than resolved
 * from a NavController, because there is no navigation graph in the project
 * yet (flagged, not guessed at). Wiring this screen into real nav-arg
 * passing is a separate next step.
 */
@Composable
fun PlayerScreen(
    ...
    DisposableEffect(Unit) {
        viewModel.play(request, profileId)
        onDispose {
    ...
    Box(
        ...
        when (coordinatorState) {
            ...
            is CoordinatorState.Error -> ErrorContent(
                ...
                onRetry = { viewModel.play(request, profileId) }
            )
        }
    }
}
